...
void LocalMapping::CreateNewMapPoints()
{
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        for(size_t ikp=0, iendkp=vMatchedKeysUn1.size(); ikp<iendkp; ikp++)
        {
            ...
            if(cosParallaxRays<0 || cosParallaxRays>0.99999998) //JMX
                continue;
            ...
            mlpRecentAddedMapPoints.push_back(pMP);
            new_key_points_num = new_key_points_num + 1; //JMX
        }
    }
    std::cout << "In CreateNewMapPoints: creat new points number = " << new_key_points_num << std::endl;//JMX
