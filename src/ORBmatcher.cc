...
int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
{
    ...
    for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)
    {
        ...
        {
            // 遍历vIndices2中 与上述点特征向量最相似的点和倒数第二相似的点
            // bestDist 和 bestDist2
            size_t i2 = *vit;
            cv::Mat d2 = F2.mDescriptors.row(i2);
            int dist = DescriptorDistance(d1,d2);
        ...
        { // 若距离向量小于TH_LOW
            ...
            {
                // 最相似距离 小于倒数第二相似距离 * mfNNratio
                if(vnMatches21[bestIdx2]>=0)
                {
                    // 若该点已和其他点匹配过, 则设置当前帧中bestIdx2对应的上次匹配的特征点vnMatches21[bestIdx2]为未匹配状态
                    vnMatches12[vnMatches21[bestIdx2]]=-1;
                    nmatches--;
                }
                vnMatches12[i1]=bestIdx2;       // 参考帧中的特征点 i1 对应当前帧的 bestIdx2.
                vnMatches21[bestIdx2]=i1;       // 当前帧中的特征点 bestIdx2 对应参考帧的 i1.
                vMatchedDistance[bestIdx2]=bestDist;      // 保留最近距离
                nmatches++;                               // 匹配点数增加

                if(mbCheckOrientation)                    // 统计旋转一致性
                {
                    float rot = F1.mvKeysUn[i1].angle-F2.mvKeysUn[bestIdx2].angle;
                    if(rot<0.0)
                        rot+=360.0f;
                    int bin = round(rot*factor);          // 划分成不同扇区
                    if(bin==HISTO_LENGTH)
                        bin=0;
                    ROS_ASSERT(bin>=0 && bin<HISTO_LENGTH);
                    rotHist[bin].push_back(i1);          // 把当前匹配点加入扇区的保存列表中    
                }
            }
        }
